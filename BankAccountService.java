...
public class BankAccountService {
	public static final Logger logger = LoggerFactory.getLogger(BankAccountService.class);
	...
	public void createAccount(String accountNumber, String ownerName, double initialBalance)
		...
		if (initialBalance < MINIMUM_INITIAL_BALANCE) {
			...
			throw new InvalidAccountException("Owner name cannot be");
		...
		if (accountExists(accountNumber)) {
			...
			logger.warn("Account creation failed: accpint {} already exist", accountNumber);
	...
	public void transfer(String fromAccountNumber, String toAccountNumber, double amount)
		...
		logger.info("Transder request: {} -> {}, amount: {}", fromAccountNumber, toAccountNumber, amount);	
		// Validate amount
		if (amount <= 0) {
			// TODO 15: Log WARN message "Transfer failed: invalid amount {}"
			// Parameter: amount
			logger.warn("Transder failed: invalid amount {}", amount);
			...
			throw new InvalidTransferException("Transfer amount must be positive. Received: %.2f" + amount);
		}
		...
		if (fromAccountNumber.equals(toAccountNumber)) {
			...
			logger.warn("Transder failed: same source and destination account {}", fromAccountNumber);
		...
		if (fromAccount == null) {
			...
			// Parameter: fromAccountNumber
			logger.warn("Transder failed: source account {} not found", fromAccountNumber);
		...
		if (toAccount == null) {
			...
			logger.warn("Transder failed: destination account {} not found", toAccountNumber);
		...
		if (fromAccount.getBalance() < amount) {
			...
			logger.warn("Transder failed: insufficient funds in account {}, Balance: {}, Required: {}", fromAccountNumber, fromAccount.getBalance(), amount);
		...
		try {
			...
			// Parameters: fromAccountNumber, toAccountNumber, amount
			logger.info("Transder successful: {} -> {}, amount: {}", fromAccountNumber, toAccountNumber, amount);	
		} catch (Exception e) {
			// TODO 26: Log ERROR message "Transfer failed due to unexpected error" with
			// exception object
			// Pass exception 'e' as last parameter
			logger.error("Transder failed due to unexpected error", e);
